# Hi there, I'm Arundhathi H 👋

### MSc Computational Biology & Bioinformatics (By Research)
**Yenepoya University, Mangalore** | Research Intern @ **REVA University, Bengaluru**

🧬 Multi-Omics Integration &nbsp;•&nbsp; 🎗️ Cancer Genomics &nbsp;•&nbsp; 🧫 Epigenomics &nbsp;•&nbsp; 🧪 Structural Bioinformatics

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/arundhathi-h-22bb48285)
[![Gmail](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:arundhathiharishchandra@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/arundhathiharishchandra-20)

</div>

---

### 🧬 About Me

I'm a research student passionate about understanding cancer biology through computational approaches. My dissertation focuses on **integrated multi-omics analysis of epigenetic modalities in Diffuse Large B-Cell Lymphoma (DLBCL)**, combining RNA-seq and Whole Genome Bisulfite Sequencing (WGBS) data. I also work on structural bioinformatics, studying protein-ligand interactions through molecular docking.

- 🔭 **Currently working on:** MOFA2 multi-omics integration of RNA-seq + WGBS for DLBCL *(in progress)*
- 🌱 **Learning:** Multi-omics integration, network biology, epigenomics
- 💬 **Ask me about:** Bioinformatics pipelines, NGS analysis, molecular docking, R/Python for genomics
- 📍 **Location:** Mangalore, Karnataka, India

---

### 🛠️ Tech Stack

<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnu-bash&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

**NGS & Pipelines:** `FastQC` `fastp` `HISAT2` `featureCounts` `Bismark` `methylKit` `STAR` `BWA` `samtools` `Trimmomatic`

**Multi-Omics & Analysis:** `edgeR` `DESeq2` `WGCNA` `MOFA2` `gprofiler2` `clusterProfiler` `Cytoscape`

**Structural Bioinformatics:** `AutoDock Vina` `PyRx` `UCSF Chimera` `PyMOL` `Swiss-Model` `ADFR Tools`

**Genomics:** `PLINK` `GWAS pipelines` `variant calling` `population genetics`

**Databases:** `NCBI` `Ensembl` `GEO` `UniProt` `ClinVar` `BLAST` `ClustalW`

</div>

---

### 🔬 Featured Projects

<table>
<tr>
<td width="50%" valign="top">

**🧬 [DLBCL Multi-Omics Analysis](https://github.com/arundhathiharishchandra-20/dlbcl-multiomics-analysis)**

RNA-seq + WGBS pipeline for multi-omics epigenetic analysis of DLBCL lymphoma (129 patient samples, PRJNA605794)

- FastQC → fastp → HISAT2 → featureCounts → edgeR
- WGCNA co-expression network (11 modules, hub genes)
- GO/KEGG pathway enrichment
- MOFA2 integration *(in progress)*

`Python` `R` `Bash` `WGCNA` `Bismark`

</td>
<td width="50%" valign="top">

**🧪 [RXRα Fatty Acid Docking](https://github.com/arundhathiharishchandra-20/rxra-fatty-acid-docking)**

In-silico investigation of fatty acid interactions with Retinoid X Receptor Alpha through homology modeling and molecular docking

- Structural modelling via UCSF Chimera
- 3D models built in Swiss-Model
- Docking of 4 fatty-acid ligands (ALA, DHA, EPA, AA)
- Binding energy & structure-activity analysis

`PyRx` `AutoDock Vina` `UCSF Chimera` `PyMOL`

</td>
</tr>
</table>

---

### 📊 GitHub Stats

<div align="center">

![](https://github-readme-stats.shion.dev/api?username=arundhathiharishchandra-20&theme=merko&hide_border=false&include_all_commits=false&count_private=false)

![](https://streak-stats.demolab.com/?user=arundhathiharishchandra-20&theme=merko&hide_border=false)

![](https://github-readme-stats.shion.dev/api/top-langs/?username=arundhathiharishchandra-20&theme=merko&hide_border=false&include_all_commits=false&count_private=false&layout=compact)

![](https://github-profile-trophy.vercel.app/?username=arundhathiharishchandra-20&theme=merko&no-frame=true&row=1&column=6)

[![](https://komarev.com/ghpvc/?username=arundhathiharishchandra-20&color=2ecc71)](https://visitcount.itsvg.in)

</div>

---

<div align="center">

*Open to research and industry opportunities in bioinformatics and computational biology.*

</div>


